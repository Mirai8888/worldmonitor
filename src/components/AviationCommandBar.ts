...
async function executeIntent(intent: Intent): Promise<CommandResult> {
    if (intent.type === 'OPS') {
      ...
      <div class="cmd-row">
        ...
        return { html: `<div class="cmd-section"><strong> Ops Snapshot</strong>${rows}</div>` };
    }
    ...
    if (intent.type === 'TRACK') {
        return { html: `<div class="cmd-section"> Tracking <strong>${escapeHtml(intent.callsign ?? intent.icao24 ?? '?')}</strong> — open Tracking tab in Airline Intel panel for live positions.</div>` };
    }

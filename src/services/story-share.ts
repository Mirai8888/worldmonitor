...
export const shareTexts = {
  twitter: (data: StoryData) =>
    ...
    `${data.threats.critical > 0 ? ` ${data.threats.critical} critical threats\n` : ''}` +
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via Seithar Monitor - Open source geopolitical intelligence`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - Seithar Monitor`)}`,
